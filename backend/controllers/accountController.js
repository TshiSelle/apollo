    ...
    if (takenUsername) {
      ...
      dbUser
            ...
            (err, token) => {
              if (err) {
                return res
                  .status(500)
                  .json({
                    message: `Error during token creation : ${err}`,
                    success: false,
                  });
              } else {
        ...
        .catch((err) => {
          ...
          console.log(
            `Error occurred while storing user account in database : ${err}`
          );
        });
    }
  }
    ...
    validUser
      ...
      .then((dbUser) => {
        res
          .status(200)
          .json({
            message: `${dbUser.username}'s account verified!`,
            success: true,
          });
      })
      .catch((err) => {
        res
          .status(400)
          .json({ error: err.name + ": " + err.message, success: false });
      });
  } else {
...
const changePassword = async (req, res) => {
  ...
  if (!isValid) {
    ...
    const currentUserFound = await User.findOne({
    ...
    if (currentUserFound) {
      ...
      bcrypt
        ...
        .then((validPassword) => {
          if (validPassword) {
            if (
              Validator.equals(changeInfo.oldPassword, changeInfo.newPassword)
            ) {
              return res
                .status(400)
                .json({
            ...
            bcrypt
              ...
              .then((hashedPassword) => {
                ...
                currentUserFound
                  ...
                  .catch((err) => {
                    console.log(
                      `Error occurred during updating user's password in DB : ${err}`
                    );
                  });
              })
              .catch((err) => {
                console.log(
                  `Error occurred while hashing user's new password : ${err}`
                );
              });
          } else {
            res
        ...
        .catch((err) => {
          console.log(
            `Error occurred while verifying user's password : ${err}`
          );
        });
    } else {
...
const forgotPassword = async (req, res) => {
  ...
  if (user) {
    ...
    if (user.verified) {
      ...
      return res
        .status(200)
        .json({ message: "Reset Password email sent", success: true });
...
const resetPassPage = async (req, res) => {
  ...
  if (dbUser) {
    ...
    res
      .status(200)
      .json({ tokenValid: true, message: "User authenticated", success: true });
  } else {
    return res
      .status(401)
      .json({
  ...
  if (!isValid) {
    ...
    if (dbUser) {
      if (!bcrypt.compareSync(resetInfo.newPassword, dbUser.password)) {
        ...
        res
          .status(400)
          .json({ message: "New password can't be same as old password." });
      }
    } else {
      return res
        .status(401)
        .json({ message: "User not found or token expired", success: false });
    }
  }
};
      ...
      if (dbUser) {
        if (dbUser.deactivationDate) {
          res
            .status(400)
            .json({
              message: "User account is already in deactivation",
              success: false,
            });
        } else {
          dbUser.deactivationDate = new Date(
      ...
      if (dbUser.deactivationDate) {
        ...
        dbUser
          ...
          .then(() => {
            res
              .status(201)
              .json({ message: "Deactivation process stopped", success: true });
          })
          .catch((err) => {
            res.status(500).json({
              message: `Error occurred while setting deactivation config : ${err}`,
        ...
        res
          .status(400)
          .json({
            message: "User account is not in deactivation process",
            succes: false,
          });
      }
    })
